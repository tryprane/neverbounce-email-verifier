            ...
            try:
                ...
                for res in results:
                    ...
                    if res.get("success"):
                        ...
                        err = res.get("error")
                        if attempt < max_retries and any(e in (err or "") for e in ("RATE_LIMITED_429", "BOT_CHALLENGE_403", "SESSION_TERMINATED_EARLY")):
...
async def main():
    async with Actor:
        ...
        if not custom_proxies:
            try:
                if proxy_input:
                    if "apifyProxyCountry" not in proxy_input:
                        proxy_input["apifyProxyCountry"] = "US"
                    ...
                    try:
                        apify_proxy_config = await Actor.create_proxy_configuration(groups=["RESIDENTIAL"], country_code="US")
